...
#         # === ANALISIS PER CHECKPOINT (USER FRIENDLY) ===
#         st.subheader("📍 Analisis Detail per Checkpoint")
        
#         # Buat tabs untuk setiap checkpoint yang bermasalah
#         problem_sources = sorted(missing_data['Source'].unique())
        
#         if len(problem_sources) <= 5:
#             # Jika checkpoint bermasalah sedikit, buat tabs
#             tabs = st.tabs([f"📍 {source}" for source in problem_sources])
            
#             for i, source in enumerate(problem_sources):
#                 with tabs[i]:
#                     source_missing = missing_data[missing_data['Source'] == source]
                    
#                     # Summary untuk checkpoint ini
#                     col1, col2 = st.columns(2)
#                     with col1:
#                         st.metric("Total Data Hilang", len(source_missing))
#                         st.metric("Tanggal Bermasalah", source_missing['Tanggal'].nunique())
#                     with col2:
#                         st.metric("Jenis Kendaraan Hilang", source_missing['Jenis Kendaraan'].nunique())
...
#                         st.metric("Kelengkapan Checkpoint", f"{source_completeness:.1f}%")
                    
#                     # Detail tanggal dan jenis kendaraan yang hilang
#                     st.write("**🗓️ Data yang Hilang per Tanggal:**")
#                     missing_by_date = source_missing.groupby('Tanggal')['Jenis Kendaraan'].apply(list).reset_index()
#                     missing_by_date['Jenis Kendaraan'] = missing_by_date['Jenis Kendaraan'].apply(lambda x: ', '.join(x))
#                     missing_by_date['Jumlah Jenis Hilang'] = source_missing.groupby('Tanggal').size().values
#                     missing_by_date = missing_by_date.sort_values('Tanggal')
                    
#                     # Tampilkan dalam format yang mudah dibaca
#                     for _, row in missing_by_date.iterrows():
#                         with st.expander(f"📅 {row['Tanggal']} - {row['Jumlah Jenis Hilang']} jenis kendaraan hilang"):
#                             jenis_list = row['Jenis Kendaraan'].split(', ')
#                             cols = st.columns(min(3, len(jenis_list)))
#                             for idx, jenis in enumerate(jenis_list):
#                                 with cols[idx % 3]:
#                                     st.write(f"🚗 {jenis}")
#         else:
#             # Jika terlalu banyak checkpoint bermasalah, gunakan selectbox
#             selected_source = st.selectbox(
#                 "Pilih Checkpoint untuk Analisis Detail:",
#                 problem_sources,
#                 format_func=lambda x: f"📍 {x}"
#             )
            
#             source_missing = missing_data[missing_data['Source'] == selected_source]
            
#             # Summary untuk checkpoint yang dipilih
#             col1, col2 = st.columns(2)
#             with col1:
#                 st.metric("Total Data Hilang", len(source_missing))
#                 st.metric("Tanggal Bermasalah", source_missing['Tanggal'].nunique())
...
#             # Detail data yang hilang dalam format tabel
#             st.write("**🗓️ Data yang Hilang per Tanggal:**")
#             missing_by_date = source_missing.groupby('Tanggal')['Jenis Kendaraan'].apply(list).reset_index()
#             missing_by_date['Jenis Kendaraan Hilang'] = missing_by_date['Jenis Kendaraan'].apply(lambda x: ', '.join(x))
#             missing_by_date['Jumlah Hilang'] = source_missing.groupby('Tanggal').size().values
#             missing_by_date = missing_by_date.sort_values('Tanggal')
#             missing_by_date = missing_by_date.drop(columns=['Jenis Kendaraan'])
...
#                     "Jumlah Hilang": st.column_config.NumberColumn("📊 Jumlah Hilang", format="%d"),
#                     "Jenis Kendaraan Hilang": st.column_config.TextColumn("🚗 Jenis Kendaraan yang Hilang")
#                 },
#                 hide_index=True,
